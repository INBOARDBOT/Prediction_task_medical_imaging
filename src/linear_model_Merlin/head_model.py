"""Linear Cox decoder head + Cox partial likelihood loss.

The head is intentionally a single linear layer (no hidden layers): its job
is just to combine radiomics and/or DINOv3 features into one risk score,
with the DINOv3 backbone doing the representation learning. No bias term --
a Cox model's baseline hazard absorbs any constant offset, so a bias on the
risk score is unidentifiable and only adds a free parameter to overfit.
"""
