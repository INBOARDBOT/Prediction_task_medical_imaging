"""Train the hybrid ViT-Cox model (Fig. 4, p.15) on the NPC cohort:
image branch = cached DINOv3 CLS tokens, radiomics branch = shallow
transformer initialized from LUNA16 masked-token pretraining. Single
train/valid/test split (lighter first pass, per project scope -- full
nested-CV validation, like the linear-head study got, is a follow-up once
this architecture is confirmed to train sensibly).

Loss: L_total = (1-lambda_loss) * L_Cox + lambda_loss * L_contrastive
Risk: F = (1-alpha) * r_img + alpha * r_rad

Run in the `dinov3` env: conda run -n dinov3 python src/ViT_cox/train.py
"""
...
def main():
    ...
    parser.add_argument("--config", type=Path, default=PROJECT_ROOT / "config" / "vit_cox_config.yaml")
